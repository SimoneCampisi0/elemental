...
public class PostConverter extends AbstractConverter<Post, PostDTO> {
    ...
    public Post toEntity(PostDTO dto) {
        Post p = null;
        if (dto != null) {
            p = new Post(dto.getIdPost(), dto.getLikes(), dto.getDataPost(), dto.getContenuto(), dto.getImgURL(), dto.getUser());
        }
        return p;
    }
    ...
    public PostDTO toDTO(Post p) {
        PostDTO dto = null;
        if (p != null) {
            dto = new PostDTO(p.getIdPost(), p.getLikes(), p.getDataPost(), p.getContenuto(), p.getImgURL(), p.getUser());
        }
        return dto;
    }
}
